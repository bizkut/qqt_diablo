...
local function logics(target, target_selector_data)
    ...
    local cast_ok, delay = my_utility.try_cast_spell("divine_lance", spell_data.divine_lance.spell_id, menu_boolean,
        next_time_allowed_cast,
        function()
            return cast_spell.target(target, spell_data.divine_lance.spell_id, spell_data.divine_lance.cast_delay,
                false)
        end,
        spell_data.divine_lance.cast_delay)
    if cast_ok then
        local current_time = get_time_since_inject();
        local cooldown = (delay or spell_data.divine_lance.cast_delay);
