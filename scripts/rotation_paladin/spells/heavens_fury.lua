...
local menu_elements =
{
    ...
    main_boolean        = my_utility.safe_checkbox(true,
        get_hash(my_utility.plugin_label .. "heavens_fury_main_bool_base")),
    targeting_mode      = my_utility.safe_combo_box(0, get_hash(my_utility.plugin_label .. "heavens_fury_targeting_mode")),

    advanced_tree       = my_utility.safe_tree_tab(2),
    priority_target     = my_utility.safe_checkbox(false,
        get_hash(my_utility.plugin_label .. "heavens_fury_priority_target")),
    ...
    elites_only         = my_utility.safe_checkbox(false, get_hash(my_utility.plugin_label .. "heavens_fury_elites_only")),
...
local function menu()
    if menu_elements.tree_tab:push("Heavens Fury") then
        menu_elements.main_boolean:render("Enable Heavens Fury", "")
...
local function logics(target, target_selector_data)
    if not target then
        if menu_elements.debug_mode:get() then
            my_utility.debug_print("[HEAVENS FURY DEBUG] No target provided")
        end
        return false
    end;
    ...
    if menu_elements.elites_only:get() and not target:is_elite() then
        if menu_elements.debug_mode:get() then
            my_utility.debug_print("[HEAVENS FURY DEBUG] Elites only mode - target is not elite")
        end
        return false
    end
    ...
    local is_logic_allowed = my_utility.is_spell_allowed(
        ...
        spell_data.heavens_fury.spell_id);

    if not is_logic_allowed then
        if menu_elements.debug_mode:get() then
            my_utility.debug_print("[HEAVENS FURY DEBUG] Logic not allowed - spell conditions not met")
        end
        return false
    end;
    ...
    if not my_utility.is_in_range(target, max_spell_range) or my_utility.is_in_range(target, menu_elements.min_target_range:get()) then
        if menu_elements.debug_mode:get() then
            my_utility.debug_print("[HEAVENS FURY DEBUG] Target not in valid range")
        end
        return false
    end
    ...
    if menu_elements.debug_mode:get() then
        my_utility.debug_print("[HEAVENS FURY DEBUG] Cast failed")
    end
    return false;
end
