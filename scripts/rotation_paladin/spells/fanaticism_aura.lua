...
local menu_elements =
{
    ...
    use_custom_cooldown = my_utility.safe_checkbox(false,
        get_hash(my_utility.plugin_label .. "fanaticism_aura_use_custom_cooldown")),
    custom_cooldown_sec = my_utility.safe_slider_float(0.1, 5.0, 0.1,
        get_hash(my_utility.plugin_label .. "fanaticism_aura_custom_cooldown_sec")),
    debug_mode          = my_utility.safe_checkbox(false,
        get_hash(my_utility.plugin_label .. "fanaticism_aura_debug_mode")),
}
...
local function logics()
    ...
    local cast_ok, delay = my_utility.try_cast_spell("fanaticism_aura", spell_data.fanaticism_aura.spell_id, menu_boolean,
        next_time_allowed_cast,
        function() return cast_spell.self(spell_data.fanaticism_aura.spell_id, spell_data.fanaticism_aura.cast_delay) end,
        spell_data.fanaticism_aura.cast_delay)

    if cast_ok then
        local current_time = get_time_since_inject();
        local cooldown = (delay or spell_data.fanaticism_aura.cast_delay);
